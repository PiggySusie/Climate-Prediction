...
def predict_by_time(target_time_str):
    # === 配置路径 ===
    scaler_path = "processed_data/scaler_with_features.save"
    model_path = "processed_data/conv_lstm_multistation_v2.h5"
    input_csv = "data/merged_stations_pm_cleaned.csv"     # 标准化过的
    true_csv = "data/merged_stations_pm.csv"              # 原始真实值 
    ...
    plot_prediction_heatmaps(pred_real, true_grid, station_grid, target_time_str,
                         save_path=f"processed_data/prediction_heatmap_{target_time_str.replace(':', '-')}.png")
